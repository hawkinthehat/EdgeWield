...
interface SmartBetEntryProps {
  ...
  onSave: (bet: SavedBet) => void;
}

export default function SmartBetEntry({
  game,
  userProfile,
  onSave,
}: SmartBetEntryProps) {
  ...
  useEffect(() => {
    const baseUnit = userProfile.total_bankroll * userProfile.unit_size_percentage;
    setCalculatedWager(baseUnit * confidence);
  }, [confidence, userProfile]);

  const bankroll = userProfile.total_bankroll || 0;
  const bankrollRatio = bankroll > 0 ? calculatedWager / bankroll : 0;
  const isHighRisk = bankrollRatio > 0.04;

  const formattedSport = useMemo(
  ...
  return (
    <div className="mx-auto w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex items-center justify-between">
        ...
        >
          <Shield size={12} />
          {isHighRisk ? "Caution" : "Shield Active"}
        </div>
      </div>
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold">
              $
              {calculatedWager.toLocaleString(undefined, {
      ...
      {isHighRisk && (
        <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-100 bg-amber-50 p-4">
          ...
          <p className="text-xs text-amber-800">
            <strong>Risk Alert:</strong> This wager represents{" "}
            <strong>{(bankrollRatio * 100).toFixed(1)}%</strong> of your bankroll.
            This exceeds your {userProfile.risk_tolerance} strategy.
          </p>
        </div>
      )}
      ...
      <button
        type="button"
        onClick={() =>
          onSave({ team: selectedTeam, amount: calculatedWager, units: confidence })
        }
